...
class OrdersController extends Controller
{
    ...
    public function create()
    {
       ...
       $order = json_decode($input, true);
           ...
           if (count($owner) > 0 && count($result) == 0) {
                            ...
                            if ($order['financial_status'] == "paid") {
                                ...
                                $result = Customer::where('email', $order['email'])
                                        ->where('customer_id', $order['customer']['id'])
                                        ->get();

                                if (count($result) > 0) {

                                   $tercero = Tercero::where('email', $result[0]['last_name'])->where('state', true)->get();

                                    if (count($tercero) > 0) {
                                       $find = Tercero::find($tercero[0]['id']);
                                       $find->total_price_orders + $order['total_price'];
    ...
    {  
       ...
       $order = json_decode($input, true);
            ...
            $owner = Customer::where('email', $order['email'])
                    ->where('customer_id', $order['customer']['id'])
                    ->first();
            ...
            if (count($owner) > 0) {
                ...
                if (count($result) > 0 ) {
                    ...
                    if ($order['financial_status'] == "paid") {
                        ...
                        $findorder = Order::where('order_id', $order['id'])
                            ->where('email', $order['email'])
                            ->where('network_id', 1)
                            ->first();
                        ...
                        if($findorder->financial_status <> "paid") {
                                           ...
                                           $product = Product::where('id', $order['line_items'][0]['product_id'])->get();
                                            ...
                                            if (count($product) > 0) {
                                                   ...
                                                   $resultaux = Customer::where('email', $order['email'])
                                                        ->where('customer_id', $order['customer']['id'])
                                                        ->get();

                                                    if (count($resultaux) > 0) {

                                                        $tercero = Tercero::where('email', $resultaux[0]['last_name'])->get();
